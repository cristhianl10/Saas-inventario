...
class ApiService {
  ...
  bool _isMissingUserIdColumnError(PostgrestException e) {
    ...
    return e.code == '42703' || (msg.contains('user_id') && msg.contains('does not exist'));
  }

  Future<T> _runWithUserIdFallback<T>(Future<T> Function(bool useUserId) operation) async {
    try {
  ...
  Future<List<Categoria>> getCategorias() async {
    return _runWithUserIdFallback((useUserId) async {
      ...
      final response = useUserId ? await query.eq('user_id', _userId).order('id') : await query.order('id');
  ...
  Future<Categoria> createCategoria(Categoria categoria) async {
    return _runWithUserIdFallback((useUserId) async {
      ...
      final response = await _client.from('categorias').insert(data).select().single();
  ...
  Future<Categoria> updateCategoria(Categoria categoria) async {
    return _runWithUserIdFallback((useUserId) async {
      var query = _client.from('categorias').update({'nombre': categoria.nombre}).eq('id', categoria.id!);
  ...
  Future<List<Producto>> getProductosPorCategoria(int categoriaId) async {
    return _runWithUserIdFallback((useUserId) async {
      var query = _client.from('productos').select().eq('categoria_id', categoriaId);
      if (useUserId) query = query.eq('user_id', _userId);
  ...
  Future<Producto> createProducto(Producto producto) async {
    return _runWithUserIdFallback((useUserId) async {
      ...
      if (useUserId) data['user_id'] = _userId;
      final response = await _client.from('productos').insert(data).select().single();
  ...
  Future<Producto> updateProducto(Producto producto) async {
    return _runWithUserIdFallback((useUserId) async {
      var query = _client.from('productos').update({
        'categoria_id': producto.categoriaId,
        'nombre': producto.nombre,
        'descripcion': producto.descripcion,
        'cantidad': producto.cantidad,
  ...
  Future<List<Venta>> getVentas() async {
    return _runWithUserIdFallback((useUserId) async {
      ...
      final response = useUserId
          ? await query.eq('user_id', _userId).order('fecha_venta', ascending: false)
  ...
  Future<Venta> createVenta(Venta venta) async {
    return _runWithUserIdFallback((useUserId) async {
      ...
      if (useUserId) data['user_id'] = _userId;
      final response = await _client.from('ventas').insert(data).select().single();
  ...
  Future<Venta> updateVenta(Venta venta) async {
    return _runWithUserIdFallback((useUserId) async {
      var query = _client.from('ventas').update({
        'producto_id': venta.productoId,
        'cantidad': venta.cantidad,
        'precio_unitario': venta.precioUnitario,
        'total': venta.total,
  ...
  Future<List<PrecioTarifa>> getTarifasPorProducto(int productoId) async {
    return _runWithUserIdFallback((useUserId) async {
      var query = _client.from('tarifa_precios').select().eq('producto_id', productoId);
      if (useUserId) query = query.eq('user_id', _userId);
  ...
  Future<PrecioTarifa> createTarifa(PrecioTarifa tarifa) async {
    return _runWithUserIdFallback((useUserId) async {
      ...
      if (useUserId) data['user_id'] = _userId;
      final response = await _client.from('tarifa_precios').insert(data).select().single();
  ...
  Future<PrecioTarifa> updateTarifa(PrecioTarifa tarifa) async {
    return _runWithUserIdFallback((useUserId) async {
      var query = _client.from('tarifa_precios').update({
        'producto_id': tarifa.productoId,
        'cantidad_min': tarifa.cantidadMin,
        'cantidad_max': tarifa.cantidadMax,
        'precio_unitario': tarifa.precioUnitario,
  ...
  Future<void> deleteTarifasPorProducto(int productoId) async {
    await _runWithUserIdFallback((useUserId) async {
      var query = _client.from('tarifa_precios').delete().eq('producto_id', productoId);
      if (useUserId) query = query.eq('user_id', _userId);
      await query;
    });
  }
  ...
  Future<List<Proveedor>> getProveedores() async {
    return _runWithUserIdFallback((useUserId) async {
      ...
      final response = useUserId ? await query.eq('user_id', _userId).order('nombre') : await query.order('nombre');
  ...
  Future<Proveedor> createProveedor(Proveedor proveedor) async {
    return _runWithUserIdFallback((useUserId) async {
      final data = <String, dynamic>{'nombre': proveedor.nombre, 'telefono': proveedor.telefono};
      if (useUserId) data['user_id'] = _userId;
      final response = await _client.from('proveedores').insert(data).select().single();
  ...
  Future<void> detachProductosFromProveedor(int proveedorId) async {
    await _runWithUserIdFallback((useUserId) async {
      var query = _client.from('productos').update({'proveedor_id': null}).eq('proveedor_id', proveedorId);
      if (useUserId) query = query.eq('user_id', _userId);
      await query;
    });
  }
